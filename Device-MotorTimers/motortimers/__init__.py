...
def create_motor(pB):
    motorName = pB.dev.params['config/name'].value
    addresses = pB.dev.params['config/addresses'].value
    if pB.value:
        if not motorName in pB.dev.params['config/motors'].value:
...
config = {
    ...
    'addresses': {
        'valueInit' : 'digout,mcp23017,smbus,1,0x20,A0:digout,mcp23017,smbus,1,0x20,B0',
        'brokerInit' : True,
        'settable' : True,
        },
    'motors': {
...
class DeviceClass(DeviceBase):
    ...
    def create_motor(self, name, addresses):
        if name in self.motors:
            ...
            if 'config/motors' in self.params:
                self.params['config/motors'].value[name] = addresses
                self.params['config/motors'].publish_value()
