...
def create_led(pB):
    ledName = pB.dev.params['config/name'].value
    address = pB.dev.params['config/address'].value
    if pB.value:
        if not ledName in pB.dev.params['config/leds'].value:
...
config = {
    ...
    'address': {
        'valueInit' : 'pwm,pca9685,smbus,1,0x40,0',
        'brokerInit' : True,
        'settable' : True,
        },
    'leds': {
...
class DeviceClass(DeviceBase):
    ...
    def create_led(self, name, address):
        if name in self.leds:
            ...
            if 'config/leds' in self.params:
                self.params['config/leds'].value[name] = address
                self.params['config/leds'].publish_value()
